...
test.beforeAll(async () => {
    ...
    for (var xmlFile = 0; xmlFile < data.length; xmlFile++) {
        var filePath = path.resolve("./tests/Author_Flow/" + config[testName]["articletotest"][xmlFile]["doi"] + ".xml")
...
test('KD-TC-5396: Author should receive a link to edit an article',async({browser}) =>{
   ...
   //url for the respective article
    await page.goto('https://staging.kriyadocs.com/proof_review?doi=Spectrum00859-23&customer=asm&project=spectrum&type=journal')
    ...
    await page.locator('#searchBox.form-control.form-control-sm').fill('Spectrum00859-23')
...
test('KD-TC-5398: Author should be able to attach pdf to the queries',async() =>{
  ...
  /*const incogo = await browser.newContext({ incognito: true });
    ...
    await newPage.goto('https://staging.kriyadocs.com/proof_review/?key=8e846e694f500fa895a00d74e40f091fc2a17eb93b81e17c454cc130eabd2d0c9f6f12296d201950e8cd7da988ca5d59d222bcb7950fc0f7b67ebfde6f688fdf0d33d0f4fca4fe709b82468fdebf92916d469cc2a30e711018dab793b3be8c5b2fba2937441d3947451beac8114dba355aee20d5daa30861c0688082bed6a1&?environment=testing')
    await newPage.waitForSelector('.articleStatus')
    */
    ...
    const unanswered_query=await newPage.locator('.query-div.card.author.unanswered')
    ...
    await fileChooser.setFiles(path.resolve("./tests/Author_Flow/author_flow 1.pdf"));
...
test('KD-TC-5397: Author should be able to resolve the opened queries',async() =>{
    ...
    //get the unanswered query
    const unanswered_q=await newPage.locator('.query-div.card.author.unanswered')
    if((expect(unanswered_q)).toBeVisible())
    {
